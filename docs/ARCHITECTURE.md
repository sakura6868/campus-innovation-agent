...
_V1.0 提交版技术架构，更新于 2026-09-14。_
...
5. 同名多年份查询优先匹配用户明确年份；未写年份且存在官方来源锚点时，显式声明使用最新届。如整体关键证据未齐，必须同时标注候选边界且不评分；连官方来源都无法锁定时才追问年份。
6. 只有 `verified + A + found + 四类关键证据完整 + 仍可报名` 的赛事可以评分；官方仅公布团队上限时，最少人数按产品规则为 1。
