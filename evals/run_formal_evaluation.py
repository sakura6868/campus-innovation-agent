...
def ground_truth_rows() -> list[tuple[Path, dict, Competition]]:
    ...
    原始 samples 是采集快照；赛事纠名、赛季归档和官方补正记录在
    curation_2026.json。数据库 seed 时会叠加这层修订，因此正式评测也
    必须使用同一份“最终真值”，避免把已经废弃的名称或日期当成期望答案。
    """
    rows = []
    curation_manifest = db._load_curation_manifest()
    for path in sorted(GT_DIR.glob("*.json")):
        raw = json.loads(path.read_text(encoding="utf-8"))
        raw = db._apply_curation(raw, curation_manifest)
        comp = db.get_competition(path.stem)
        if comp is None:
            raise AssertionError(f"数据库缺少 {path.stem}")
        rows.append((path, raw, comp))
    if len(rows) < 179:
        raise AssertionError(f"预期至少179份 Ground Truth，实际{len(rows)}份")
    return rows
...
def agent_uses_unique_canonical_deadline() -> str:
    ...
    for competition_id in core_ids:
        ...
        if comp is None or comp.registration_deadline is None:
            raise AssertionError(f"{competition_id} 缺少可评测的报名截止日期")
        result = run_agent("报名截止日期是什么时候", competition_id=competition_id, top_k=4)
        expected = comp.registration_deadline.isoformat()
        # 过期赛事会自然地回答“已于 YYYY-MM-DD 截止”，不能再用旧版
        # “报名截止日期：YYYY-MM-DD”字面格式作为唯一通过条件。
        answer_body = result["answer"].split("——— 引用来源 ———", 1)[0]
        deadline_lines = [line for line in answer_body.splitlines() if "报名截止" in line]
        if len(deadline_lines) != 1:
            raise AssertionError(f"{competition_id}: 未输出唯一的报名截止结论")
        dates = re.findall(r"\d{4}-\d{2}-\d{2}", deadline_lines[0])
        if dates != [expected]:
            raise AssertionError(f"{competition_id}: {dates} != [{expected}]")
    return "4/4 个核心赛事在当前赛季状态下均输出唯一规范报名截止日期"
...
def load_core_evidence() -> list[tuple[str, dict]]:
    # A 级且已核验的记录才进入“推荐级证据”评测。B/C 级基础资料可以
    # 用于导航与提醒，但不应被包装成推荐级证据覆盖率。
    return [
        (path.stem, raw)
        for path, raw, _ in ground_truth_rows()
...
def missing_national_deadline_is_not_invented() -> str:
    ...
    # 接受“未给出全国统一日期”等同义表达，重点验证系统明确说明能力
    # 边界，而非约束一字不差的自然语言模板。
    if "全国统一" not in answer or not any(marker in answer for marker in ("未给出", "未公布", "暂无")):
        raise AssertionError("未明确说明官方文件缺少全国统一日期")
    if re.search(r"报名截止(?:日期)?[：:]\s*\d{4}-\d{2}-\d{2}", answer):
