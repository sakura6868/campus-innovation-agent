...
在上述可信底座上，V1.0 将 GitHub 优秀项目中的产品模式做了轻量、独立实现：
...
- **质量驾驶舱**：`#/admin` 以可视化页面展示 15 项正式指标、59 项回归、86 条金标问答与安全门禁；`GET /api/system/quality` 保留机器可读契约。冻结评测与当前运行指标分开展示，避免把历史成绩冒充实时数据。
