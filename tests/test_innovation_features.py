...
class RadarTests(unittest.TestCase):
    ...
    def test_demo_scan_creates_baseline_then_pending_change(self) -> None:
        cid = f"radar_action_{uuid4().hex}"
        db.upsert_competition(_ready_comp(404, 60, ["Python"]).model_copy(update={"competition_id": cid}))
        watch = db.create_source_watch(
            cid,
            f"https://example.edu/notice-{uuid4().hex}",
        )
        self.addCleanup(self._cleanup_competition_watches, cid)
    ...
    @staticmethod
    def _cleanup_competition_watches(competition_id: str) -> None:
        with db.session_scope() as session:
            watch_ids = [
                row[0]
                for row in session.query(db.SourceWatchModel.watch_id)
                .filter_by(competition_id=competition_id)
                .all()
            ]
            if watch_ids:
                session.query(db.SourceChangeEventModel).filter(
                    db.SourceChangeEventModel.watch_id.in_(watch_ids)
                ).delete(synchronize_session=False)
                session.query(db.SourceSnapshotModel).filter(
                    db.SourceSnapshotModel.watch_id.in_(watch_ids)
                ).delete(synchronize_session=False)
                session.query(db.SourceWatchModel).filter(
                    db.SourceWatchModel.watch_id.in_(watch_ids)
                ).delete(synchronize_session=False)
            session.query(db.CitationModel).filter_by(competition_id=competition_id).delete()
            session.query(db.CompetitionModel).filter_by(competition_id=competition_id).delete()
    ...
    def test_proxy_resolved_public_hostname_uses_public_dns_fallback(self) -> None:
        proxy_answer = [(None, None, None, None, ("10.0.0.8", 443))]
        with patch("radar.service.socket.getaddrinfo", return_value=proxy_answer), patch(
            "radar.service._resolve_public_dns", return_value={"203.0.113.9"}
        ) as resolve_public:
            self.assertEqual(
                validate_public_url("https://contest.example.edu/notice"),
                "https://contest.example.edu/notice",
            )
        resolve_public.assert_called_once_with("contest.example.edu")

    def test_proxy_resolved_private_hostname_remains_rejected(self) -> None:
        proxy_answer = [(None, None, None, None, ("10.0.0.8", 443))]
        with patch("radar.service.socket.getaddrinfo", return_value=proxy_answer), patch(
            "radar.service._resolve_public_dns", return_value=set()
        ):
            with self.assertRaises(ValueError):
                validate_public_url("https://private.example.edu/notice")
    ...
    def test_image_only_pdf_uses_fingerprint_fallback(self) -> None:
        with patch("radar.service._normalize_pdf", return_value=""):
            normalized, kind = _normalise(b"%PDF-1.4 image-only", "application/pdf", "pdf", None)
        self.assertEqual(kind, "pdf")
        self.assertIn("图像型 PDF", normalized)
        self.assertIn("仅比较文件指纹", normalized)

    def test_script_shell_page_uses_dynamic_fingerprint_fallback(self) -> None:
        shell = b"<html><head><script src='/app.js'></script></head><body></body></html>"
        normalized, kind = _normalise(shell, "text/html", "auto", None)
        self.assertEqual(kind, "dynamic")
        self.assertIn("脚本渲染页面", normalized)
        self.assertIn("仅比较原始响应指纹", normalized)

    def test_dynamic_page_change_requires_review_without_field_claims(self) -> None:
        watch = db.create_source_watch(
            "china_softcup_2026",
            f"https://example.edu/dynamic-{uuid4().hex}",
            source_type="dynamic",
            trigger_terms=["报名", "截止"],
        )
        self.addCleanup(self._cleanup_watch, watch["watch_id"])
        first = run_watch(watch["watch_id"], "<main>动态赛事页版本一</main>")
        second = run_watch(watch["watch_id"], "<main>动态赛事页版本二</main>")
        self.assertEqual(first["status"], "baseline_created")
        self.assertEqual(second["status"], "change_detected")
        self.assertEqual(second["change_type"], "dynamic_page_changed")
        self.assertEqual(second["severity"], "low")
        self.assertEqual(second["affected_fields"], [])

    def test_ended_competition_keeps_a_weekly_maintenance_watch(self) -> None:
        cid = f"radar_maintenance_{uuid4().hex}"
        comp = _ready_comp(402, 60, ["Python"]).model_copy(
            update={
                "competition_id": cid,
                "document_year": date.today().year - 1,
        ...
        self.addCleanup(self._cleanup_competition_watches, cid)
        with db.session_scope() as session:
            db.seed_source_watches(session)

        stored = next(item for item in db.list_source_watches() if item["competition_id"] == cid)
        self.assertTrue(stored["enabled"])
        self.assertEqual(stored["monitor_tier"], "maintenance")
        self.assertEqual(stored["interval_hours"], 168)
        self.assertIn("获奖", stored["trigger_terms"])

    def test_maintenance_change_stays_manual_and_never_proposes_fact_updates(self) -> None:
        cid = f"radar_maintenance_change_{uuid4().hex}"
        comp = _ready_comp(403, 60, ["Python"]).model_copy(
            update={
                "competition_id": cid,
                "document_year": date.today().year - 1,
                "registration_deadline": date.today() - timedelta(days=1),
        ...
        watch = db.create_source_watch(
            cid,
            f"https://example.edu/maintenance-{uuid4().hex}",
            trigger_terms=["通知", "报名"],
        )
        self.addCleanup(self._cleanup_competition_watches, cid)

        first = run_watch(watch["watch_id"], "2025 年赛事资料归档")
        second = run_watch(
            watch["watch_id"],
            "2027 年赛事报名通知：报名截止时间为 2027 年 09 月 30 日",
        )
        self.assertEqual(first["status"], "baseline_created")
        self.assertEqual(second["status"], "change_detected")
        self.assertEqual(second["change_type"], "maintenance_source_changed")
        self.assertEqual(second["severity"], "low")
        self.assertEqual(second["affected_fields"], [])
        self.assertEqual(second["proposed_changes"], {})

        third = db.create_change_event(
            watch["watch_id"],
            second["snapshot_id"],
            second["snapshot_id"],
            severity="low",
        ...
        self.assertTrue(third.get("deduplicated"))
        pending = [
            item
            for item in db.list_change_events(status="pending")
            if item["watch_id"] == watch["watch_id"]
        ]
        self.assertEqual(len(pending), 1)

    def test_seed_does_not_duplicate_an_existing_alternate_watch(self) -> None:
        cid = f"radar_alternate_{uuid4().hex}"
        comp = _ready_comp(401, 60, ["Python"]).model_copy(update={"competition_id": cid})
        db.upsert_competition(comp)
        watch = db.create_source_watch(
            cid,
            f"https://example.edu/alternate-{uuid4().hex}",
        )
        self.addCleanup(self._cleanup_competition_watches, cid)
        with db.session_scope() as session:
            db.seed_source_watches(session)
        with db.session_scope() as session:
            watches = session.query(db.SourceWatchModel).filter_by(competition_id=cid).all()
            self.assertEqual(len(watches), 1)
            self.assertEqual(watches[0].watch_id, watch["watch_id"])

    def test_seed_rotates_a_stale_watch_when_official_source_changes(self) -> None:
        cid = f"radar_rotated_{uuid4().hex}"
        comp = _ready_comp(404, 60, ["Python"]).model_copy(
            update={
                "competition_id": cid,
                "official_source_url": f"https://example.edu/current-{uuid4().hex}.pdf",
            }
        )
        db.upsert_competition(comp)
        old_watch = db.create_source_watch(
            cid,
            f"https://example.edu/old-{uuid4().hex}.pdf",
        )
        self.addCleanup(self._cleanup_competition_watches, cid)
        with db.session_scope() as session:
            stale = session.get(db.SourceWatchModel, old_watch["watch_id"])
        ...
        with db.session_scope() as session:
            ...
            old = next(item for item in watches if item.watch_id == old_watch["watch_id"])
            current = next(item for item in watches if item.source_url == comp.official_source_url)
            self.assertFalse(old.enabled)
            self.assertEqual(old.last_status, "paused")
            self.assertTrue(current.enabled)
            self.assertEqual(current.consecutive_failures, 0)

    def test_seed_uses_reviewed_radar_override_without_rewriting_fact_source(self) -> None:
        with db.session_scope() as session:
            db.seed_source_watches(session)
        comp = db.get_competition("supply_chain_2026")
        watch = next(
            item
            for item in db.list_source_watches()
            if item["competition_id"] == "supply_chain_2026" and item["enabled"]
        )
        self.assertEqual(
            comp.official_source_url,
            "https://supplychain.chinawuliu.com.cn/",
        )
        self.assertEqual(
            watch["source_url"],
            "https://jw.wzbc.edu.cn/UploadFile/202603/202603301522526497.pdf",
        )
        self.assertEqual(watch["source_type"], "pdf")
