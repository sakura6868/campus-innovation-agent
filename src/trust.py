...
def assess_source_readiness(comp: Competition) -> ReadinessAssessment:
    ...
    Some nationwide contests are organised by province, track, or a rolling
    certification calendar.  They legitimately have no single national
    registration deadline, so a missing date must not turn otherwise usable
    basic information into a "candidate only" record.  The exact schedule is
    still shown in the event's official-note field and users are reminded to
    check their own region or track.
    """
    reasons: list[str] = []
