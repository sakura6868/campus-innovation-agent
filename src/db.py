...
CURATION_MANIFEST_PATH = PROJECT_ROOT / "data" / "ground_truth" / "curation_2026.json"
RADAR_SOURCE_OVERRIDES_PATH = PROJECT_ROOT / "data" / "radar_sources.json"
...
class CompetitionModel(Base):
    ...
    # archived（已核查但不应进入当前赛事大厅，仍保留审计记录）
...
def _load_curation_manifest() -> dict:
    """读取赛事纠名与归档清单；缺失时安全地视为无附加规则。"""
    if not CURATION_MANIFEST_PATH.exists():
        return {}
    try:
        return json.loads(CURATION_MANIFEST_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return {}


def _load_radar_source_overrides() -> dict[str, dict]:
    """读取人工复核过的雷达补充入口。

    该配置只决定“监控哪个可访问页面”，不会改写赛事主表的
    ``official_source_url``，更不会绕过人工审核更新任何事实。
    """
    if not RADAR_SOURCE_OVERRIDES_PATH.exists():
        return {}
    try:
        payload = json.loads(RADAR_SOURCE_OVERRIDES_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return {}
    ...
    if not isinstance(sources, dict):
        return {}
    return {
        str(competition_id): value
        for competition_id, value in sources.items()
        if isinstance(value, dict)
        and isinstance(value.get("source_url"), str)
        and value["source_url"].startswith(("https://", "http://"))
    }


def _apply_curation(raw: dict, manifest: dict) -> dict:
    """叠加可追溯的数据纠名与归档结论，不篡改原始样本文件。"""
    item = dict(raw)
    competition_id = item.get("competition_id") or Path(item.get("_source_file", "")).stem
    corrections = manifest.get("corrections") or {}
    if isinstance(corrections.get(competition_id), dict):
        item.update(corrections[competition_id])

    archived = {
        entry.get("competition_id"): entry
        for entry in (manifest.get("archived") or [])
        if isinstance(entry, dict) and entry.get("competition_id")
    }
    audit = archived.get(competition_id)
    if audit:
        reviewed_at = manifest.get("reviewed_at") or date.today().isoformat()
        item.update(
            {
                "official_source_status": "archived",
                "official_source_url": None,
                "source_acquired_date": reviewed_at,
                "trusted_level": "C",
                ...
                "evidence": [],
                "doc_version": f"{item.get('document_year', 2026)}_archived_20260730",
                "notes": (
                    f"【2026-07-30 归档】{audit.get('reason', '未能定位当前赛季官方来源。')}"
                    + (f" 建议关注：{audit['replacement']}。" if audit.get("replacement") else "")
                    + " 此记录仅保留审计，不进入赛事大厅、推荐、项目创建或智能问答。"
                ),
            }
        )
    return item
...
def seed_competitions(session) -> int:
    ...
    if not GROUND_TRUTH_DIR.exists():
        return 0
    count = 0
    curation_manifest = _load_curation_manifest()
    # macOS 解压/拷贝时可能产生 ``._*.json`` AppleDouble 资源文件；它们不是
    # JSON 数据，不能参与赛事灌库，否则 Linux/Docker 首次启动会解码失败。
    for f in sorted(
        path for path in GROUND_TRUTH_DIR.glob("*.json") if not path.name.startswith(".")
    ):
        raw = json.loads(f.read_text(encoding="utf-8"))
        raw.setdefault("competition_id", f.stem)
        raw["_source_file"] = str(f)
        _upsert_competition_from_raw(_apply_curation(raw, curation_manifest), session)
        count += 1
    return count
...
_ACTION_TRIGGER_TERMS = ["报名", "截止", "参赛资格", "团队", "赛道", "提交"]
_MAINTENANCE_TRIGGER_TERMS = ["通知", "公告", "报名", "截止", "赛程", "结果", "获奖", "规则", "附件"]


def _is_actionable_for_radar(comp: CompetitionModel, today: date) -> bool:
    """是否应进入高频行动提醒层，而不是是否值得长期监视。"""
    if comp.document_year != today.year:
        return False
    if comp.competition_start_date and comp.competition_start_date <= today:
        return False
    if comp.competition_end_date and comp.competition_end_date < today:
        return False
    if comp.registration_deadline and comp.registration_deadline < today:
        return False
    if (
        comp.registration_deadline is None
        and comp.submission_deadline
        and comp.submission_deadline < today
    ):
        return False
    return True


def _monitor_tier_for_competition(comp: CompetitionModel, today: Optional[date] = None) -> str:
    """Return the operational monitoring tier for a traceable competition source."""
    if comp.official_source_status != "found" or not comp.official_source_url:
        return "unavailable"
    current = today or date.today()
    return "action" if _is_actionable_for_radar(comp, current) else "maintenance"


def _watch_interval_for_competition(comp: CompetitionModel, today: date) -> int:
    """High-frequency monitoring is reserved for actionable deadlines.

    Maintenance watches still compare their sources, but run weekly to catch
    next-season notices, result releases and rule supplements without turning
    historical pages into a noisy alert stream.
    """
    if _monitor_tier_for_competition(comp, today) == "maintenance":
        return 168
    deadline_days = (
        (comp.registration_deadline - today).days
        if comp.registration_deadline is not None
        else None
    )
    return 6 if deadline_days is not None and deadline_days <= 14 else (12 if deadline_days is not None else 24)


def _is_replaced_duplicate_watch(watch: SourceWatchModel) -> bool:
    return (
        not watch.enabled
        and watch.last_status == "paused"
        and "稳定替代入口接管" in (watch.last_error or "")
    )


def seed_source_watches(session) -> None:
    """为全部可追溯赛事建立双层来源监视，归档来源保持停用。

    行动层只服务当前可报名赛事，采用高频轮询并影响提醒与规划；资料维护层
    保留已结束、往届和已截止赛事的低频监视，用于发现新赛季公告、补充通知
    或结果发布。两层都只生成待审核情报，绝不自动覆盖赛事事实。
    """
    today = date.today()
    radar_overrides = _load_radar_source_overrides()

    # 已归档或已失去来源的记录不再访问；保留历史快照和失败审计。
    for watch, comp in (
        session.query(SourceWatchModel, CompetitionModel)
    ...
    for comp in rows:
        tier = _monitor_tier_for_competition(comp, today)
        interval_hours = _watch_interval_for_competition(comp, today)
        override = radar_overrides.get(comp.competition_id, {})
        # 补充入口仅用于稳定监控。赛事详情页仍展示主表中的官方来源，
        # 雷达变化也仍需人工审核后才可能影响结构化事实。
        preferred_source_url = override.get("source_url") or comp.official_source_url
        preferred_source_type = override.get("source_type") or (
            "pdf" if ".pdf" in (preferred_source_url or "").lower() else "auto"
        )
        existing_watches = (
            session.query(SourceWatchModel)
            .filter(SourceWatchModel.competition_id == comp.competition_id)
            .order_by(SourceWatchModel.watch_id)
            .all()
        )
        candidates = [watch for watch in existing_watches if not _is_replaced_duplicate_watch(watch)]
        if not candidates:
            session.add(
                SourceWatchModel(
                    ...
                    trigger_terms=_dump_json(
                        _ACTION_TRIGGER_TERMS if tier == "action" else _MAINTENANCE_TRIGGER_TERMS
                    ),
        ...
        # 保留人工选择的稳定入口；优先已有健康快照的记录，避免把备用链接
        # 或已明确停用的重复项重新拉入扫描队列。
        primary = max(
            candidates,
            key=lambda watch: (
                int(watch.enabled),
                int(watch.last_success_at is not None),
                int(watch.last_status == "ok"),
                watch.last_checked_at or datetime.min,
                -watch.watch_id,
            ),
        )

        # 官方来源经纠名/复核后发生变化时，不能让已持续失败的旧 URL 一直
        # 占用主监控位。保留旧监控及其快照作为审计记录，同时创建新的官方
        # 入口建立基线。只有连续失败且明确 stale/error 的入口才自动迁移，
        # 因而不会替换人工配置的健康备用来源。
        has_preferred_source_watch = any(
            watch.source_url == preferred_source_url for watch in candidates
        )
        should_rotate_failed_source = (
            not has_preferred_source_watch
            and primary.source_url != preferred_source_url
            and primary.last_status in {"stale", "error"}
            and primary.consecutive_failures >= 3
        )
        if should_rotate_failed_source:
            primary.enabled = False
            primary.last_status = "paused"
            primary.last_error = "官方来源已更新，已切换到当前复核入口并保留旧快照审计"
            session.add(
                SourceWatchModel(
                    competition_id=comp.competition_id,
                    source_url=preferred_source_url,
        ...
        primary.interval_hours = interval_hours
        if primary.last_status == "paused":
            primary.last_status = "new"
            primary.last_error = None
        existing_terms = _load_json(primary.trigger_terms)
        if tier == "maintenance" and (
            not existing_terms or existing_terms == _ACTION_TRIGGER_TERMS
        ):
            # 旧版本曾只对可报名赛事配置行动关键词。赛事转入资料维护层后，
            # 还需要捕捉公告、获奖和新赛季赛程；保留人工自定义的其他词表。
            primary.trigger_terms = _dump_json(_MAINTENANCE_TRIGGER_TERMS)
        # 重新启用的历史来源在下一批中补建/刷新快照；已连续运行的来源
        # 仍沿用既有 next_scan_at，避免每次启动都对所有官网突发请求。
        if not was_enabled:
            primary.next_scan_at = None
...
def list_competitions(
    ...
    with session_scope() as session:
        ...
        query = query.filter(
            or_(
                CompetitionModel.official_source_status.is_(None),
                CompetitionModel.official_source_status != "archived",
            )
        )
        if category:
...
def _watch_dict(
    item: SourceWatchModel,
    competition_name: Optional[str] = None,
    monitor_tier: Optional[str] = None,
) -> dict:
    ...
    return {
        ...
        "monitor_tier": monitor_tier or "unavailable",
...
def create_source_watch(
    ...
    with session_scope() as session:
        ...
        return _watch_dict(
            ...
            _monitor_tier_for_competition(comp),
        )


def list_source_watches(enabled_only: bool = False) -> list[dict]:
    with session_scope() as session:
        query = session.query(SourceWatchModel, CompetitionModel).join(
        ...
        return [
            _watch_dict(watch, comp.competition_name, _monitor_tier_for_competition(comp))
            for watch, comp in query.order_by(SourceWatchModel.watch_id).all()
        ]
...
def get_source_watch(watch_id: int) -> Optional[dict]:
    with session_scope() as session:
        ...
        return (
            _watch_dict(row[0], row[1].competition_name, _monitor_tier_for_competition(row[1]))
            if row
            else None
        )
...
def create_change_event(
    ...
    with session_scope() as session:
        # 资料维护层只提供“可能有新赛季信息”的人工线索，不携带可写回
        # 的结构化字段。相同来源在上一条线索仍待审核时继续产生同类事件，
        # 只会淹没审核队列，因此合并为一条待办；新的快照仍会完整保存。
        if change_type == "maintenance_source_changed" and not affected_fields and not (proposed_changes or {}):
            existing = (
                session.query(SourceChangeEventModel)
                .filter(
                    SourceChangeEventModel.watch_id == watch_id,
                    SourceChangeEventModel.change_type == change_type,
                    SourceChangeEventModel.status == "pending",
                )
                .order_by(SourceChangeEventModel.event_id.desc())
                .first()
            )
            if existing is not None:
