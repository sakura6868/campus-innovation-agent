...
class Competition(BaseModel):
    ...
    # —— 官方来源核实结论 ——
    # found：已定位官方来源；not_found：待查证；archived：经核查不属于当前
    # 可展示的全国赛事（旧赛事、地方赛、重复项或名称无法纠正），保留审计记录。
    official_source_status: Optional[str] = None  # "found" | "not_found" | "archived" | None
...
class RadarWatchCreate(BaseModel):
    competition_id: str
    source_url: str
    source_type: Literal["html", "pdf", "docx", "dynamic", "auto"] = "auto"
