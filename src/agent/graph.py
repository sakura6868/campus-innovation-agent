...
def _requested_recommendation_count(question: str, default: int = 3) -> int:
    """识别“推荐三场/给我 2 个”等数量要求；未指定时默认给 3 个重点选择。"""
    text = str(question or "")
    match = re.search(r"(?:推荐|给我|挑选|选择|对比)\s*([一二三四五六七八九十\d]+)\s*(?:场|个|项)", text)
    if not match:
        return default
    raw = match.group(1)
    chinese = {"一": 1, "二": 2, "三": 3, "四": 4, "五": 5, "六": 6, "七": 7, "八": 8, "九": 9, "十": 10}
    value = int(raw) if raw.isdigit() else chinese.get(raw, default)
...
def node_recommend_all(state: AgentState) -> AgentState:
    ...
    requested_count = _requested_recommendation_count(state.get("question") or "")
    formal = [r for r in results if r.eligible][:requested_count]
    selected = formal
    ...
    trace.append(
        f"全库推荐：共检查 {len(results)} 条，按用户要求返回 {len(formal)} 条；候选信息仅在赛事目录展示"
    )
...
def node_compose(state: AgentState) -> AgentState:
    ...
    if intent == "team":
        ...
        if not recos:
            ...
            lines = [f"结合你的画像和当前报名状态，我建议优先比较以下 {len(recos)} 场赛事：\n"]
            for i, r in enumerate(recos, 1):
                ...
                gap = str(explanation.get("能力缺口") or "").replace("缺少技能：", "需要补充：")
                suffix = f"；{gap}" if gap and not gap.endswith("无") else ""
                lines.append(
                    f"{i}. {r['competition_name']} — 匹配度 {sc}，{status_map.get(r['recommendation_status'], r['recommendation_status'])}{suffix}{mark}{tag_s}"
                )
            first = recos[0]
            if len(recos) > 1:
                alternatives = "、".join(item["competition_name"] for item in recos[1:])
                lines.append(
                    f"\n取舍建议：优先深入查看「{first['competition_name']}」，它的当前综合匹配度最高；"
                    f"「{alternatives}」作为备选，先核对能力缺口和实际投入时间后再决定。"
                )
            else:
                lines.append(f"\n取舍建议：先核对「{first['competition_name']}」的最新通知和自身时间，再决定是否加入项目。")
            lines.append("\n这些建议基于现有画像和已收录资料，报名前仍请查看赛事官网最新通知。")
            answer = "\n".join(lines)
        answer += _citations_appendix(citations)
        ...
        question = state.get("question") or ""
        def _fact_mark(*fields: str) -> str:
            for index, citation in enumerate(citations):
                if citation.get("field") in fields:
                    return _cite_marker(index)
            return ""
        if comp is None:
            answer = "没能定位到具体赛事，请补充赛事名称。"
        else:
            facts = [f"关于「{comp.competition_name}」，明确结论如下："]
            if _asks_team_size(question):
                team_mark = _fact_mark("team_min", "team_max")
                if not comp.team_required or (comp.team_min == 1 and comp.team_max == 1):
                    facts.append(f"1. 参赛形式：个人参赛，队伍人数为 1 人{team_mark}。")
                else:
                    facts.append(f"1. 队伍人数：{format_team_size(comp.team_min, comp.team_max)}{team_mark}。")
            deadline_mark = _fact_mark("registration_deadline")
            number = 2 if _asks_team_size(question) else 1
            if comp.registration_deadline is None:
                facts.append(f"{number}. 报名截止：官方文件未给出全国统一日期{deadline_mark}。")
            elif comp.registration_deadline < date.today():
                facts.append(f"{number}. 报名截止：已于 {comp.registration_deadline.isoformat()} 截止{deadline_mark}。")
            else:
                facts.append(f"{number}. 报名截止：{comp.registration_deadline.isoformat()}{deadline_mark}。")
            answer = "\n".join(facts)
        answer += _citations_appendix(citations)
